...
export class UbigeoFacade {
  ...
  preload() {
    // Si ya tenemos data, no hacemos nada (evita parpadeos)
  ...
  departamentos$ = this.data$.pipe(
    map(data => this.toOptions([...new Set(data.map(x => x.departamento))]))
  );

  provincias$(departamento: string) {
    return this.data$.pipe(
      map(data => data.filter(x => x.departamento === departamento)),
      map(rows => this.toOptions([...new Set(rows.map(x => x.provincia))]))
    );
  }
  ...
  //       x.distrito !== null)),
  //     map(rows => this.toOptions(
  //       rows
  //         .map(x => ({ value: x.ubigeo, label: x.distrito }))
  //         .filter((v, i, arr) => arr.findIndex(a => a.value === v.value) === i)
  //         .map(x => `${x.value}||${x.label}`) // truco para unique
  //     ).map(o => {
  //       const [value, label] = o.value.split('||');
  //       return { value, label };
  //     }))
  //   );
  // }
